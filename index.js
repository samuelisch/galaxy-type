...
const checkKey = (e) => {
  ...
  const instance = displayedChars.find(
    (charInstance) => charInstance.text === char
  );
  if (instance) {
    matchCounter++;
...
const viewLeaderboard = () => {
  ...
  document
    .querySelectorAll(".leaderboard-score")
    .forEach((score) => score.remove());
...
const removeFromDisplayArray = (charId) => {
  displayedChars = displayedChars.filter(
    (charInstance) => charInstance.id !== charId
  );
};
...
const newChar = () => {
  ...
  const actualHeight =
    char.metrics.actualBoundingBoxAscent +
    char.metrics.actualBoundingBoxDescent;
  let accumulateY = 0;
